...
import me.poodar.uis.mf.utils.BPRMFThread;
import me.poodar.uis.mf.utils.CLiMFThread;
...
public class CLiMultiTest {
  ...
  public static void main(String args[]) throws Exception {
    ...
    List<Future<CLiMF>> mfResults = new ArrayList<>();
    int counter = 0 ;
    if(args[1].equals("2") ) {
      int sysTheadSize = 8 ;
      for(int j = 0 ; j < Parameter.L / sysTheadSize + 1 ; j++) {
        for (int i = 0; i < sysTheadSize ; i++) {
          ...
          CLiMFThread mfThread = new CLiMFThread(recommender, 
              Parameter.IFMFPath +medium+".CLi."+t_no , 
        ...
        for (int i = 0; i < sysTheadSize ; i++) {
          int t_no = j * 8 + i;
          if(t_no >= Parameter.L) break;
          Future<CLiMF> mf = mfResults.get(t_no);
          try {
            mf.get();
    ...
    for (int i = 0; i < Parameter.L; i++) {
      ...
      training_data_list.add(training_data);
      CLiMF recommender = new CLiMF();
      recommender.loadModel(Parameter.IFMFPath + medium + ".CLi." + i);
  ...
  public static void saveprec(String medium, IPosOnlyFeedback testData,
      CLiMF recommender, int i, IPosOnlyFeedback training_data)
      throws IOException {
  ...
  public static ItemRecommendationEvaluationResults evaluate(
    ...
    String pathString = Parameter.maxF1Path + medium + ".CLi."+ Parameter.L + ".I"+Parameter.iL;
