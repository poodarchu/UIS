...
public class FileUtil {
  ...
  public static void readLinesFromString(String text, ArrayList<String> lines) {
...
//		inputstring = "fds fds Won't won't can't Can't ain't";
	...
	public static void tokenizeAndLowerCase(String line, ArrayList<String> tokens) {
		...
		while (strTok.hasMoreTokens()) {
			String token = strTok.nextToken();
			//String tokenWithoutPunctuation = token.toLowerCase().trim();
			tokens.add(token.toLowerCase().trim());
		}
	}
	
	 public static void tokenizeAndLowerCaseAndPunctuationRemoval
	   (String line, ArrayList<String> tokens ,String punctuation) {
	    // TODO Auto-generated method stub
	    StringTokenizer strTok = new StringTokenizer(line);
	    while (strTok.hasMoreTokens()) {
	      String token = strTok.nextToken();
	      String tokenWithoutPunctuation = token.toLowerCase().trim();
	      tokens.add(tokenWithoutPunctuation.replaceAll("[^a-zA-Z ]", ""));
	    }
	  }
	 
   public static void sentenceTokenizedStemmedFipped
   (String review, ArrayList<String> tokens ,Stemmer stemmer) {
    // TODO Auto-generated method stub
    review = review.replaceAll("[^a-zA-Z ]", " ");
    StringTokenizer strTok = new StringTokenizer(review);
    while (strTok.hasMoreTokens()) {
      String token = strTok.nextToken();     
      token= token.toLowerCase().trim();
      //remove stopwords
      while(Stopwords.isStopword(token)) {
        if (strTok.hasMoreTokens()) {
          token = strTok.nextToken();     
          token= token.toLowerCase().trim();
        } else {
          //if all words behind are stopwords , jump out
          return;
        }
      }
      if(token.equals("not")||token.equals("cannot")||
          token.equals("don")||token.equals("never") ||
          token.equals("doesn")) {
        if (strTok.hasMoreTokens()) {
          token = strTok.nextToken();
          token = token.toLowerCase().trim();
          //get the first stopwords after not and flip it
          while(Stopwords.isStopword(token)) {  
            if (strTok.hasMoreTokens()) {
                token = strTok.nextToken();     
                token= token.toLowerCase().trim();
      ...
      //tokens.add(stemmer.stemString(token));
      //eliminate s suffix
      /*
      if (token.endsWith("s")) {
